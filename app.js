...
(function () {
  ...
  const state = {
    ...
    verifyPath: null, // "asistida" | "proveedor"
    passId: null,
    ticket: null,
    qrNonce: null,
    qrExpiresAt: null,
  ...
  function drawQrPattern(canvas, seed) {
    ...
    // Simple deterministic pseudo-QR (visual only, not a real QR codec)
    let h = 0;
  ...
  function startTtl() {
    ...
    ttlTimer = setInterval(() => {
      ...
      if (state.qrExpiresAt && Date.now() >= state.qrExpiresAt) {
        // Auto-rotate for demo convenience after short pause
      }
    }, 250);
  }
  function stopTtl() {
  ...
  // Expose for manual demo / console
  window.FanectDemo = {
    go,
    state,
    rotateQr,
